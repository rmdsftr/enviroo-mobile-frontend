...
class _DetailProfilNasabahScreenState extends State<DetailProfilNasabahScreen> {
  ...
  Future<void> _fetchDetail() async {
    ...
    await prov.fetchNasabah(widget.nasabahId);
    if (!mounted) return;
    if (prov.nasabahStatus == FetchStatus.success) {
      setState(() { _detail = prov.nasabah; _isLoading = false; });
    } else {
      setState(() { _error = prov.nasabahError ?? 'Gagal memuat profil'; _isLoading = false; });
    }
  }
    ...
    return Column(
      children: [
        _buildSectionContainer(
          ...
          children: [
            _buildInfoRow('ID Pengguna', _detail!['user_id']?.toString() ?? '-'),
