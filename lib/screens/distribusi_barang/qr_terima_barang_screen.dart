...
class QrTerimaBarangScreen extends StatefulWidget {
  const QrTerimaBarangScreen({super.key});

  @override
  State<QrTerimaBarangScreen> createState() => _QrTerimaBarangScreenState();
}

class _QrTerimaBarangScreenState extends State<QrTerimaBarangScreen>
  ...
  void _onDetect(BarcodeCapture capture) async {
    ...
    if (!raw.startsWith('ENVIROO-DISTRIBUSI|')) return;

    final disbaId = raw.split('|')[1];
    if (disbaId.isEmpty) return;

    setState(() => _isProcessing = true);
    _scannerController.stop();
    HapticFeedback.mediumImpact();

    await _submit(disbaId);
  }

  Future<void> _submit(String disbaId) async {
    ...
    final result = await barang.addDistribusiFromQr(
      disbaId: disbaId,
      bsuId: auth.bankId ?? '',
      adminBsuId: auth.identityId ?? '',
    );
    ...
    if (result['success'] == true) {
      ...
      showCustomSnackBar(context, result['message'] ?? 'Gagal menerima distribusi');
  ...
  Future<void> _showSuccessDialog() async {
    await showSuccessBottomSheet(
      ...
      message: 'Distribusi barang berhasil diterima.',
      buttonLabel: 'Selesai',
    ...
    return Scaffold(
      body: Container(
        ...
        child: SafeArea(
          child: Column(
            children: [
              const TopBarBack(title: 'Scan QR Distribusi Barang'),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'Scan QR distribusi dari petugas BSI untuk mengkonfirmasi penerimaan barang.',
              style: TextStyle(
                fontFamily: 'Poppins',
                fontSize: 11.5,
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              _isSubmitting ? 'Memproses Distribusi...' : 'Memverifikasi QR...',
            ...
            Text(
              _isSubmitting ? 'Mohon tunggu sebentar' : 'Memproses data distribusi',
              style: TextStyle(
                fontFamily: 'Poppins',
                fontSize: 12,
