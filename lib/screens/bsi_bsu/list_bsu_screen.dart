...
class _ListBsuScreenState extends State<ListBsuScreen> {
  ...
  Future<void> _fetchBsu() async {
    ...
    await prov.fetchBsuList(bankId, paksa: true);

    if (!mounted) return;

    if (prov.bsuStatus == FetchStatus.success) {
      setState(() {
        _bsuList = prov.bsuList;
        _isLoading = false;
      });
    } else {
      setState(() {
        _error = prov.bsuError ?? 'Gagal memuat data BSU';
        _isLoading = false;
      });
    }
  }
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.error_outline_rounded, size: 52, color: _C.dark.withValues(alpha: 0.2)),
            const SizedBox(height: 16),
            Text(
              _error,
              textAlign: TextAlign.center,
  ...
  Widget _buildCardBsu(BsuUnitModel bsu) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) =>
              ProfilBankScreen(bankId: bsu.bankId, judul: 'Profil Bank Sampah'),
        ),
      ),
      child: Container(
