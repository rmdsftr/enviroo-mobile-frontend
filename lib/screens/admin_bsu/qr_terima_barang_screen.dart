...
class QrTerimaSembakoScreen extends StatefulWidget {
  const QrTerimaSembakoScreen({super.key});

  @override
  State<QrTerimaSembakoScreen> createState() => _QrTerimaSembakoScreenState();
}

class _QrTerimaSembakoScreenState extends State<QrTerimaSembakoScreen>
  ...
  void _onDetect(BarcodeCapture capture) async {
    ...
    final disbakoId = raw.split('|')[1];
    if (disbakoId.isEmpty) return;

    setState(() => _isProcessing = true);
    _scannerController.stop();
    HapticFeedback.mediumImpact();

    await _submit(disbakoId);
  }

  Future<void> _submit(String disbakoId) async {
    ...
    final result = await sembako.addDistribusiFromQr(
      disbakoId: disbakoId,
      bsuId: auth.bankId ?? '',
