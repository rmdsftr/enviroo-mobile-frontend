...
class StrukDistribusiSembakoScreen extends StatefulWidget {
  final String disbakoId;
  const StrukDistribusiSembakoScreen({super.key, required this.disbakoId});

  @override
  State<StrukDistribusiSembakoScreen> createState() =>
      _StrukDistribusiSembakoScreenState();
}

class _StrukDistribusiSembakoScreenState
    extends State<StrukDistribusiSembakoScreen> {
  bool _loading = true;
  String? _error;
  DetailDistribusiSembakoModel? _detail;
  ...
  Future<void> _load() async {
    ...
    final res = await SembakoService.getDetailDistribusi(widget.disbakoId);
    if (!mounted) return;
    if (res['success'] == true) {
      setState(() {
        _detail = DetailDistribusiSembakoModel.fromJson(
  ...
  // ── Info Card ──────────────────────────────────────────────────────────────
  Widget _buildInfoCard(DetailDistribusiSembakoModel d) {
    final status = d.statusDistribusi.toLowerCase();
    final Color statusColor;
    final String statusLabel;
  ...
  // ── Total Poin Card ────────────────────────────────────────────────────────
  Widget _buildTotalCard(DetailDistribusiSembakoModel d, NumberFormat fmt) {
    return Container(
      width: double.infinity,
  ...
  // ── Items Card ─────────────────────────────────────────────────────────────
  Widget _buildItemsCard(DetailDistribusiSembakoModel d, NumberFormat fmt) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
