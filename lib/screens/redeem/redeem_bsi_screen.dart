...
import 'detail_verifikasi_bsi_screen.dart';
import 'scan_redeem_screen.dart';

class RedeemBsiScreen extends StatefulWidget {
  const RedeemBsiScreen({super.key});

  @override
  State<RedeemBsiScreen> createState() => _RedeemBsiScreenState();
}

class _RedeemBsiScreenState extends State<RedeemBsiScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            const TopBarBack(title: 'Redeem Masuk'),
            Padding(
  ...
  Widget _buildList() {
    return Consumer<RedeemBsiProvider>(
  ...
  void _onTapCard(RedeemTransaksi item) {
    if (item.status == RedeemStatus.waiting) {
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) =>
              DetailVerifikasiBsiScreen(transaksiId: item.transaksiId),
        ),
      );
    }
  }
  ...
  void _showWaitingActions(RedeemTransaksi item) {
    ...
    ).then((_) => context.read<RedeemBsiProvider>().loadList());
  }

  void _openScanner() async {
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const ScanRedeemScreen()),
    );
    if (mounted) context.read<RedeemBsiProvider>().loadList();
  }
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              ...
              onPressed: () => context.read<RedeemBsiProvider>().loadList(),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                _row('Asal', item.bankAsal?.namaBank ?? '-'),
