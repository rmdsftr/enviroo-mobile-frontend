...
import '../../providers/redeem_bsi_provider.dart';
import '../../widgets/redeem_card.dart';
import '../../widgets/topbar_back.dart';

class DetailVerifikasiBsiScreen extends StatefulWidget {
  final String transaksiId;
  const DetailVerifikasiBsiScreen({super.key, required this.transaksiId});

  @override
  State<DetailVerifikasiBsiScreen> createState() =>
      _DetailVerifikasiBsiScreenState();
}

class _DetailVerifikasiBsiScreenState extends State<DetailVerifikasiBsiScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final prov = context.read<RedeemBsiProvider>();
      prov.bind(auth);
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              status == 'success'
                  ? 'Saldo BSU akan dikonversi dan transaksi diselesaikan. Lanjutkan?'
                  : 'Yakin ingin membatalkan redeem ini?',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Consumer<RedeemBsiProvider>(
          builder: (context, prov, _) {
            return Stack(
              children: [
                Column(
                  children: [
                    const TopBarBack(title: 'Verifikasi Redeem'),
                    Expanded(
      ...
      child: Column(
        ...
        children: [
          ...
          _section('Informasi Transaksi', [
            ...
            _row('Asal (BSU)', item.bankAsal?.namaBank ?? '-'),
            _row('Tujuan (BSI)', item.bankTujuan?.namaBank ?? '-'),
  ...
  Widget _buildActions(RedeemTransaksi item, bool busy) {
    ...
    final canBatalkan = isApproved || isWaiting;
    final canKonversi = isApproved;
        ...
        children: [
              ...
              children: [
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    ...
                    onPressed: (!canKonversi || busy)
                        ? null
                        : () => _confirmAction(
                              status: 'success',
                              title: 'Konversi Saldo',
          ...
          if (isWaiting)
            Padding(
              ...
              child: Text(
                'Status masih waiting — terima dulu di list utama agar BSU bisa menampilkan QR.',
                textAlign: TextAlign.center,
                style: TextStyle(
                  fontFamily: 'Poppins',
                  fontSize: 11,
