...
class InputRequestRedeemScreen extends StatefulWidget {
  const InputRequestRedeemScreen({super.key});

  @override
  State<InputRequestRedeemScreen> createState() =>
      _InputRequestRedeemScreenState();
}

class _InputRequestRedeemScreenState extends State<InputRequestRedeemScreen> {
  ...
  final Map<String, double> _sembakoQty = {}; // sembako_id -> qty
  ...
  Future<void> _submit() async {
    ...
    if (reward.isSembako) {
      ...
      if (prov.saldo != null && totalPoin > prov.saldo!.saldoPoin) {
        _toast('Total poin sembako melebihi saldo BSU', error: true);
        return;
      }
      ...
      if (prov.saldo != null && poin > prov.saldo!.saldoPoin) {
        _toast('Poin melebihi saldo BSU', error: true);
        return;
      }
      ok = await prov.submitRequest(
        rewardId: reward.rewardId,
    ...
    if (!mounted) return;
    if (ok) {
      _toast('Pengajuan redeem berhasil dikirim');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Consumer<RedeemBsuProvider>(
          builder: (context, prov, _) {
            return Stack(
              children: [
                Column(
                  children: [
                    const TopBarBack(title: 'Ajukan Redeem'),
                    Expanded(
  ...
  Widget _buildContent(RedeemBsuProvider prov) {
    return Column(
  ...
  Widget _buildSaldoCard(SaldoBank? saldo) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [dark.withOpacity(0.95), const Color(0xFF2D5A1D).withOpacity(0.9)],
        ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (saldo != null && saldo.namaBank.isNotEmpty)
                  Text(
                    saldo.namaBank,
                    style: TextStyle(
                      fontFamily: 'Poppins',
  ...
  Widget _buildRewardSelector(List<NilaiRewardBank> rewards) {
    if (rewards.isEmpty) {
      return Container(
        ...
        child: const Text(
          'BSI belum menyediakan jenis reward.',
        ...
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: selected ? primary : Colors.black.withOpacity(0.15),
              ),
  ...
  Widget _buildDynamicForm(RedeemBsuProvider prov) {
    final reward = _selectedReward!;
    if (reward.isSembako) {
  ...
  Widget _buildPoinForm(NilaiRewardBank reward) {
    ...
    final konversiText = reward.isUang
        ? 'Estimasi: Rp ${_formatNumber(konversi.round())}'
        : reward.isEmas
            ? 'Estimasi: ${konversi.toStringAsFixed(4)} ${reward.satuan}'
            : 'Estimasi: ${konversi.toStringAsFixed(2)} ${reward.satuan}';
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      'Konversi: ${_formatNumber(reward.nilaiPoin)} pts = ${reward.isUang ? 'Rp ${_formatNumber(reward.nilaiKonversi.round())}' : '${reward.nilaiKonversi} ${reward.satuan}'}',
                      style: TextStyle(
                        fontFamily: 'Poppins',
                        fontSize: 11,
  ...
  Widget _buildSembakoForm(RedeemBsuProvider prov) {
        ...
        if (items.isEmpty)
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(14),
              border: Border.all(color: const Color(0xFFFAA324).withOpacity(0.3)),
            ),
            child: const Text(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${_formatNumber(item.poinHargaBsu)} pts • Stok ${_formatNumber(item.stok)}',
                  style: TextStyle(
                    fontFamily: 'Poppins',
                    fontSize: 11,
  ...
  Widget _buildSubmitButton(RedeemBsuProvider prov) {
